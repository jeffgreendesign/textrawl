-- Textrawl Database Schema (Ollama Version)
-- Use this instead of setup-db.sql when using Ollama embeddings (1024 dimensions)
...
-- Chunks table with embeddings (1024 dimensions for Ollama models)
...
drop function if exists public.hybrid_search(text, vector(1024), int, float, float, int);
