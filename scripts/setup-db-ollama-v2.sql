...
-- Chunks table with embeddings (768 dimensions for nomic-embed-text-v2-moe)
create table if not exists chunks (
  ...
  end_offset integer,
  embedding vector(768), -- nomic-embed-text-v2-moe dimension (Matryoshka: supports 768, 512, 256)
  metadata jsonb default '{}',
  created_at timestamptz default now()
);
...
drop function if exists public.hybrid_search(text, vector(768), int, float, float, int);
