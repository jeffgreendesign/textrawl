...
function extractMetadata(doc: Document, url?: string): Partial<WebpageMetadata> {
	...
	metadata.url = url || canonicalLink?.getAttribute('href') || ogUrl?.getAttribute('content') || '';
	...
	if (dateMeta) {
		const dateStr = dateMeta.getAttribute('content') || dateMeta.getAttribute('datetime');
		if (dateStr) {
			try {
	...
	metadata.language = htmlLang || langMeta?.getAttribute('content') || undefined;
