...
export async function getMemoryStats(): Promise<{
	...
	const [entitiesResult, observationsResult, relationsResult, typeCountsResult] = await Promise.all(
		[
			client.from('memory_entities').select('*', { count: 'exact', head: true }),
			client.from('memory_observations').select('*', { count: 'exact', head: true }),
			client.from('memory_relations').select('*', { count: 'exact', head: true }),
