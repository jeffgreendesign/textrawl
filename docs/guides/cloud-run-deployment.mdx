...
[`infra/gcs/`](https://github.com/jeffgreendesign/textrawl/tree/main/infra/gcs) —
...
- The Cloud Run runtime service account needs `roles/storage.objectAdmin` on the
  bucket (granted via ADC — no keys).
- Bucket **CORS** must allow the dashboard origin with methods `PUT/POST/GET/HEAD`
  and headers `Content-Type, Content-Range, x-goog-*`.
- A **lifecycle** rule deletes abandoned objects after 1 day; soft-delete is
  disabled so deleted upload bytes are not retained/billed.
