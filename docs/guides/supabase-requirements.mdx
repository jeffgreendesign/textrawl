...
Supabase Pro plan ($25/mo) does not include a compute upgrade by default. Add a compute add-on based on your usage.
...
-- Ollama v1 (1024 dimensions)
CREATE INDEX IF NOT EXISTS chunks_embedding_idx ON chunks
USING hnsw (embedding vector_cosine_ops);

-- Ollama v2 (768 dimensions)
CREATE INDEX IF NOT EXISTS chunks_embedding_idx ON chunks
USING hnsw (embedding vector_cosine_ops);
```

The HNSW index definition is the same regardless of dimensions -- PostgreSQL infers the dimension from the column type.
...
2. **Index bloat happens.** Run `REINDEX TABLE documents;` periodically if your GIN index grows disproportionately large relative to data size (10:1+ ratio is a sign of bloat).
...
4. **Consider pgvectorscale for 100K+ vectors.** DiskANN indexes use SSD instead of RAM, making them much cheaper to scale. Supabase supports this natively via the `pgvectorscale` extension.
