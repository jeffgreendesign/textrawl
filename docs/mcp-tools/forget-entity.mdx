...
1. Use `query_memory` with mode="entity" to review what will be deleted
...
| Entity not found | No entity exists with the provided name | Verify entity name using `query_memory` with mode="list" |
...
- [`query_memory`](/docs/mcp-tools/query-memory) - Find entities (mode="list") or review before deletion (mode="entity")
- [`get_stats`](/docs/mcp-tools/get-stats) - Check entity counts (scope="memory")
